...
We show an example analysis using simulation data that resemble the BMI analysis for GECCO presented in the paper \cite{Dai2018}.
...
\cite{Dai2018}. When there are many SNPs to be evaluated as candidate instrumental variables, parallel computing can be deployed
...
out=glide(formula=formula,exposure_coeff=coeff,genotype_columns,data=simdat,
          np=100000,qcutoff=0.2,parallel=FALSE,corenumber=1,verbose=TRUE)
head(out)
@
%@<<fig=TRUE,echo=FALSE, width='16in'>>=
%@@
